...
Индекс `files_v2` (версионирован — см. §4.6), `_id` = SHA-256 от абсолютного пути файла (детерминированный ключ — переиндексация того же файла обновляет тот же документ, а не создаёт дубликат).

| Поле | Тип ES | Назначение |
|---|---|---|
| `path` | `keyword` | абсолютный путь, уникальный ключ |
| `fileName` | `text` (analyzer `russian`) + `keyword` subfield | поиск и точная фильтрация по имени |
...
### 4.6 Морфологический поиск (русский язык)

Elasticsearch поставляет встроенный анализатор `russian` (snowball-стеммер) — без плагинов, достаточно указать `analyzer = "russian"` на полях `content` и `fileName` в маппинге. `SearchService` не требует изменений: `multi_match` применяет тот же анализатор к поисковому запросу автоматически, так что "документами" в тексте находится по запросу "документ" и наоборот.

Так как Elasticsearch не позволяет сменить анализатор существующего поля без пересоздания индекса, имя индекса версионировано (`files_v2` вместо `files`) — это проще и надёжнее миграции маппинга: новый код просто стартует с чистого индекса, а старый (если был) остаётся неиспользуемым и не мешает.
