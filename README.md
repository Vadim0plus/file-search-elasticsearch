...
Приложение индексирует файлы на диске (текст, PDF, DOCX, XLSX, PPTX и др.) и даёт быстрый полнотекстовый поиск по содержимому с морфологией русского языка, подсветкой совпадений, предпросмотром, метаданными и метками файлов — в том числе генерируемыми по содержимому через AI (Claude/Anthropic API). Индекс остаётся актуальным в реальном времени: изменения файлов на диске (в том числе загруженных прямо через интерфейс) подхватываются автоматически, без ручной переиндексации.

Полная спецификация и обоснование архитектурных решений — в [SPEC.md](SPEC.md).

## Демо

Живой пример развёрнут на **http://v0plus.run.place** — вход `admin` / `admin`.

## Как разрабатывается проект

Проект пишется в паре с [Claude Code](https://claude.com/product/claude-code) (агентный CLI от Anthropic) — от спеки (`SPEC.md`) и кода до тестов и этого README; коммиты в истории репозитория несут `Co-Authored-By: Claude`. Разработка идёт в песочницах Asylum: каждая задача поднимается в отдельном git worktree, проверяется локальным `docker compose up` (включая ручное/браузерное тестирование фичи) и только после этого мержится в `main`.

## Демонстрация

![Вход, живой поиск с подсветкой, предпросмотр файла с метками и управление индексом](docs/screenshots/demo.gif)

Сценарий записи: вход → поиск «Elasticsearch» по мере ввода с подсветкой совпадений → предпросмотр файла с метаданными и ручным добавлением метки → вкладка «Управление индексом».
...
- Метки на файлах: генерация меток на основе содержимого через Claude (Anthropic API), ручное добавление/удаление, фильтр поиска по меткам
...
Генерация меток через AI требует ключ Anthropic API — задать переменную окружения `ANTHROPIC_API_KEY` перед запуском (`ANTHROPIC_API_KEY=sk-ant-... docker compose up --build`). Без ключа приложение работает как обычно, но кнопка «Сгенерировать с помощью AI» вернёт ошибку 503 — ручное добавление/удаление меток и поиск по ним при этом не затронуты.
...
| `GET` | `/api/search?q=&extension=&tag=&path=&from=&to=&page=&size=` | Полнотекстовый поиск с подсветкой |
| `GET` | `/api/files/{id}` | Метаданные, метки и извлечённый текст файла (для предпросмотра) |
...
| `GET` | `/api/tags` | Список всех меток в индексе с числом документов (для фильтра/автодополнения) |
| `POST` | `/api/files/{id}/tags/generate` | Сгенерировать метки по содержимому файла через Claude (503, если `ANTHROPIC_API_KEY` не задан) |
| `POST` | `/api/files/{id}/tags` | Добавить метку вручную (`{"tag": "договор"}`) |
| `DELETE` | `/api/files/{id}/tags/{tag}` | Удалить метку |
...
Backend поднимется на `localhost:8080`. Переменные окружения: `SPRING_ELASTICSEARCH_URIS`, `APP_CORS_ALLOWED_ORIGINS`, `APP_AUTH_USERNAME`, `APP_AUTH_PASSWORD`, `ANTHROPIC_API_KEY`, `APP_AI_MODEL` (см. `application.yml`).
...
└── docs/
    ├── screenshots/           # скриншоты и gif-демонстрация для этого README
...
- Генерация меток по запросу пользователя отправляет имя файла и часть извлечённого содержимого (до `app.ai.max-content-chars`, по умолчанию 8000 символов) в Anthropic API — не включайте эту функцию (не задавайте `ANTHROPIC_API_KEY`) для директорий с конфиденциальными данными, если это не приемлемо.
